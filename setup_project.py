...
def safe_write(
    ...
    if os.path.exists(path):
        print(f"    [skip]     {path}  (already exists — untouched)")
        stats["skipped"] += 1
        return
...
def create_structure() -> None:
    ...
    print("NOTE: Existing files will NEVER be overwritten.\n")
    ...
    print("Done.")
    print(
        f"  Directories : {stats['dirs_created']} created, "
        f"{stats['dirs_existed']} already existed"
    )
    print(
        f"  Files       : {stats['created']} created, "
...
def print_tree() -> None:
    ...
    for folder in structure:
        ...
        for index, filename in enumerate(all_files):
            connector = "└──" if index == len(all_files) - 1 else "├──"
